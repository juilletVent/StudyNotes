...
fn main() {
  // 注意：文件本身也是模块系统的一环，所以这里出现了两个 add、sub
  println!("1 + 2 = {}", utils::add::add(1, 2));
  println!("1 - 2 = {}", utils::sub::sub(1, 2));
}
```
...
在 Rust 1.30 版本之后，更推荐采用新的模块声明方式进行模块声明，文件结构将如下所示：
...
在 front_of_house 同级目录里创建一个与模块（目录）同名的 rs 文件 front_of_house.rs，然后在该文件中声明模块：
